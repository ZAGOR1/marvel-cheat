...
void Box3D(SDK::APlayerController* PlayerController, SDK::FVector Origin, SDK::FVector Extents, ImU32 Col)
{
    ...
    for (int i = 0; i < 4; ++i) {
        if ((IsOnScreen(ScreenPoints[i]) || IsOnScreen(ScreenPoints[(i + 1) % 4])) &&
            ...
            DrawList->AddLine(ScreenPoints[i], ScreenPoints[(i + 1) % 4], Col);
        }
        if ((IsOnScreen(ScreenPoints[i + 4]) || IsOnScreen(ScreenPoints[(i + 1) % 4 + 4])) &&
            ...
            DrawList->AddLine(ScreenPoints[i + 4], ScreenPoints[(i + 1) % 4 + 4], Col);
        }
        if ((IsOnScreen(ScreenPoints[i]) || IsOnScreen(ScreenPoints[i + 4])) &&
            ...
            DrawList->AddLine(ScreenPoints[i], ScreenPoints[i + 4], Col);
        }
    }
}

namespace SDK {
    const FVector FVector::OneVector = { 1.0f, 1.0f, 1.0f }; // Uygun de�erlerle ba�lat�n	
}
...
void Menu() {
  ...
  if (ImGui::BeginTabBar(_X("MenuTabs"))) {
    ...
    if (ImGui::BeginTabItem(_X("Aimbot"))) {
	  ...
	  ImGui::Checkbox(_X("TPBullet"), &Settings::Aimbot::bbullet_tp);
      ImGui::Checkbox(_X("Rage"), &Settings::Aimbot::bRage);
      ImGui::Checkbox(_X("Prediction"), &Settings::Aimbot::bPrediction);
      ImGui::Checkbox(_X("Filter by Lower Health"), &Settings::Aimbot::bFocusSquish);
    ...
    if (ImGui::BeginTabItem(_X("ESP"))) {
      ...
      if (Settings::ESP::bBox) {
          ImGui::Checkbox(_X("Standart"), &Settings::ESP::bStandart);
          ImGui::SameLine();
          ImGui::Checkbox(_X("Glow"), &Settings::ESP::bGlow);
          ImGui::SameLine();
          ImGui::Checkbox(_X("3D"), &Settings::ESP::b3D);

          if (Settings::ESP::bStandart) {
              Settings::ESP::bGlow = false;
              Settings::ESP::b3D = false;
          }
          if (Settings::ESP::bGlow) {
              Settings::ESP::bStandart = false;
              Settings::ESP::b3D = false;
          }
          if (Settings::ESP::b3D) {
              Settings::ESP::bStandart = false;
	  ...
	  ImGui::SliderFloat(_X("Fov Changer"), &Settings::ESP::bFovChanger, 80.0f, 150.0f, "%.f", 0);
    ...
    if (ImGui::BeginTabItem(_X("Exploits"))) {
      ...
      ImGui::Checkbox(_X("Rapid Fire"), &Settings::Exploits::bRapid);
      ImGui::EndTabItem();
    }
...
void Shitbot() {
  ...
  for (int i = 0; i < players.Num(); i++) {
    ...
    __try {
      ...
      if (Settings::Aimbot::bbullet_tp && closestActor) {
          ...
          if (!IsValidPtr(closestActor)) {
              return;
          }

          SDK::FVector closest_actor_forward = closestActor->GetActorForwardVector();

          auto TargetPosition = actorHead + (closest_actor_forward * 10);
          ...
          for (int i = 0; i < Bullets.Num(); i++)
          {
              ...
              if (!IsValidPtr(Bullet)) continue;

              SDK::FHitResult fhit;
              Bullet->K2_SetActorLocationAndRotation(TargetPosition, SDK::FRotator(0, 0, 0), false, &fhit, false);
          }
      }
  ...
  // Change FOV
  if (IsValidPtr(G::pLocalPlayer->PlayerCameraManager)) {
        float* pDefaultFOV = reinterpret_cast<float*>(reinterpret_cast<uintptr_t>(G::pLocalPlayer->PlayerCameraManager) + 0x4AC + 0x4);
        if (Settings::ESP::bFovChanger) {
            *pDefaultFOV = Settings::ESP::bFovChanger;
        }
  }
    ...
    if (playerController->ProjectWorldLocationToScreen(rootLoc, &Bottom,
      ...
      float distance =
          sqrt(powf(delta.X, 2) + powf(delta.Y, 2) + powf(delta.Z, 2)) / 1000.f;
            ...
            if (Settings::ESP::bBox && Settings::ESP::bStandart)
                DrawBox(head.X - (cornerWidth / 2), head.Y, cornerWidth, cornerHeight,
                        bVisible ? ImColor(0, 255, 0) : ImColor(255, 0, 0), 1.0f);
   
               
            if (Settings::ESP::bBox && Settings::ESP::b3D) {
                SDK::FVector Origin = baseClass->K2_GetActorLocation();
                SDK::FVector Extents = { 50.0f, 50.0f, 100.0f }; // Adjust the extents as needed
                Box3D(playerController, Origin, Extents, bVisible ? ImColor(0, 255, 0) : ImColor(255, 0, 0));
            }

            // Fonksiyonu global d�zeyde veya uygun bir namespace/s�n�f i�inde tan�mlay�n.
               // E�er ESP checkbox'lar� aktif de�ilse, fonksiyondan ��k.
